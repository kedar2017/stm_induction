...
(PA.05 mapped to LED2) with two different frequencies.
...
If the state of PB.00 is high then the adafruit 1.8" TFT shield is available. 
  ...
  2-2 SD Card available:
      ...
      - If the content of the SD Card is other than a bitmap file, a message will 
         ...
         * Bit depth:  16
         * Item type: BMP file
         * Root: the bmp images have be stored under the uSD root
         * File name: file name's length (with .bmp extension) must not exceed 11 characters                   
...
@par Hardware and Software environment
  ...
  - Adafruit 1.8" TFT shield must be connected on CN5,CN6, CN8 and CN9 Arduino connectors, 
    for more details please refer to STM32_Nucleo Getting started doc and
    NUCLEO-XXXXRX LQFP64 STM32 Nucleo kits doc(UM1724).  
...
In order to make the program work, you must do the following :
 ...
 + EWARM
    - Open the STM32F302_Nucleo_Demo.eww workspace 
    - Rebuild all files: Project->Rebuild all
    - Load project image: Project->Debug
    - Run program: Debug->Go(F5)

 + MDK-ARM
    - Open the STM32F302_Nucleo_Demo.uvproj project
 ...
 + TrueSTUDIO:
    ...
    - Browse to the TrueSTUDIO workspace directory, select the project STM32F302_Nucleo_Demo.
